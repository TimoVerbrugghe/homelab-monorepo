---
applyTo: '**/Dockerfile,**/Dockerfile.*,**/*.dockerfile,**/docker-compose*.yml,**/docker-compose*.yaml,**/compose*.yml,**/compose*.yaml,docker/**/*.yaml,docker/**/*.yml,docker/**/Dockerfile,docker/**/Dockerfile.*,docker/**/*.dockerfile'
...
## Homelab Addendum: Compose Hardening Conventions

The following conventions apply to Docker Compose workloads in this repository. They extend (not replace) the general best practices above.

### Required hardening anchor in every compose file

Define a YAML anchor at the top of each Compose file and merge it into every service:
...
### Resource limits anchor

Every service must set CPU, memory, and PID limits. Use a separate anchor and tune per workload:
...
- Default to `read_only: true` for every service.
- Only disable it for services that genuinely require writes at startup (e.g. Rails apps, certain s6-overlay images). When you disable it, add a short inline comment explaining why.

### Non-root user defaults

- For LinuxServer.io images, use `user: "${PUID:-1000}:${PGID:-1000}"`.
- For other images, prefer a non-root UID:GID matching the image's expected user.
- If an image must run as root (e.g. s6-overlay init), treat it as an explicit, documented exception.

### tmpfs for /tmp instead of /dev/shm

Do not bind-mount `/dev/shm` (host shared memory) as the container's `/tmp`. It bypasses tmpfs size limits and exposes host memory. Use a proper tmpfs declaration instead:
...
### Capability additions when cap_drop is ALL

Keep `cap_drop: ["ALL"]` and add only what is strictly required:

| Use case | Capabilities to add |
| --- | --- |
| DNS / web server binding port <1024 | `NET_BIND_SERVICE` |
| AdGuardHome-style config + privileged port | `NET_BIND_SERVICE`, `DAC_OVERRIDE`, `DAC_READ_SEARCH` |
| s6-overlay init (e.g. linuxserver/jlesage images) | `CHOWN`, `FOWNER`, `DAC_OVERRIDE`, `SETUID`, `SETGID` |
| File chown at startup only | `CHOWN`, `FOWNER` |

### CRITICAL: YAML anchor override pitfall

In Docker Compose, a service-level key entirely replaces the corresponding key from a merged anchor. There is no partial/deep merge.

```yaml
# WRONG: this silently drops ALL capability restrictions from the anchor
myservice:
  <<: *hardening-defaults
  cap_drop: []

# CORRECT: re-include the anchor value and only add what's required
myservice:
  <<: *hardening-defaults
  cap_drop:
    - ALL
...
Whenever a service overrides a key that exists in `<<: *hardening-defaults`, verify the override re-states the intended baseline.

The same pitfall applies if the hardening anchor itself is missing required keys (e.g. an anchor without `cap_drop: ["ALL"]` provides no capability restriction at all). Audit the anchor first, then each service override.

### Validation checklist before committing

- Each service merges `<<: *hardening-defaults`.
- No service overrides `cap_drop` to an empty list.
- Resource limits (`pids_limit`, `mem_limit`, `cpus`) are set.
- Logging is `json-file` with `max-size: "50m"` and `max-file: "5"`.
- `read_only: true` is enabled unless an exception is documented inline.
- No `/dev/shm` host mount is used as `/tmp`.
- `docker compose -f <file> config` parses successfully.
