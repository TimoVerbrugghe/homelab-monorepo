...
Official `crowdsecurity/crowdsec` Helm chart: LAPI (ban decision store, no
persistence) and agent DaemonSet (tails Traefik's JSON access logs via
hostPath, needs `privileged` Pod Security). Traefik enforces bans via the
